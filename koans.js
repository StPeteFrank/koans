...
test('Are similar strings with different delimiters equal?', t => {
  ...
  t.is('apple' === 'apple', doubleQuotedString === singleQuotedString)
})
...
test('What is the default case of a switch statement?', t => {
  ...
  //Also a guess because i like hobbits.
  t.is('Merry', hobbit)
})
...
}) //because shift "pops" from the front of an array
...
}) //It obviously does but not sure why.
...
}) //map .replace the integers with strings.
...
test('Use filter to return array items that meet a criteria', t => {
  ...
  t.deepEqual([1, 3], odd)
  t.is(2, odd.length)
}) //only solved this using the terminal undefined responses.
...
test('Use reduce to update the same result on each iteration', t => {
  ...
  t.is(6, sum)
  t.deepEqual([1, 2, 3], numbers)
}) //only got 6 through adding the three number. still dont get reduce.
